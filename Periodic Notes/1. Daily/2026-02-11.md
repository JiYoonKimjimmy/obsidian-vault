...
```
Redis Stream Consumer 에서 redis.stream.batch-size 인 10개의 메시지를 읽어올텐데 해당 메시지를 money api 호출하는 로직을 병렬로 처리하고 싶어.

그랬을 때, Result 를 update 하는 과정에서 DB connection 이 밀릴수도 있기 때문에, 
@CampaignPromotionResultUpdateService 클래스에서 ResultEntity 업데이트 로직을 모두 Async Event Listener 로 비동기 처리로 전환하자.
```
- **진행 완료**
