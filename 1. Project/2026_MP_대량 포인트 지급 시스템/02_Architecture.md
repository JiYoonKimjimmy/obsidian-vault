...
flowchart TB
    ...
    subgraph Cache["📊 Redis"]
        R[지급 현황 카운트<br/>total/published/success/fail]
    end
    
    subgraph Kafka["📨 Kafka Topic: payment-request"]
        G0[Partition 0]
        G1[Partition 1]
        G2[Partition 2]
        G3[Partition 3]
...
flowchart TB
    subgraph Normal["정상 흐름"]
        ...
        B --> C[money API 호출]
        C --> D{성공?}
        D -->|Yes| E[DB 상태 업데이트<br/>SUCCESS]
    end
    
    subgraph DLT["DLT 재처리 흐름"]
        D -->|No| F[DLT 발행]
        ...
        I --> J[money API 호출]
        J --> K{성공?}
        K -->|Yes| L[DB 상태 업데이트<br/>SUCCESS]
        K -->|No| M{retry < 5?}
        M -->|Yes| F
        M -->|No| N[Final DLT 발행]
    end
    
    subgraph Final["최종 실패"]
